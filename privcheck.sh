...
{    echo "Downloading and extracting scripts..."
    `wget -O ~/admin/privcheck/ExploitSuggest.py http://www.securitysift.com/download/linuxprivchecker.py`
    `wget -O ~/admin/privcheck/LinEnum.zip https://github.com/rebootuser/LinEnum/archive/master.zip`
    `wget -O ~/admin/privcheck/ExploitSuggest_perl.zip https://github.com/jondonas/linux-exploit-suggester-2/archive/master.zip`
    `wget -O ~/admin/privcheck/unixprivesc.zip https://github.com/pentestmonkey/unix-privesc-check/archive/1_x.zip`
    `wget -O ~/admin/privcheck/firmwalker.zip https://github.com/craigz28/firmwalker/archive/master.zip`
    for zip in *.zip
    do
...
do
    case $opt in
        ...
        "Download")
            ...
            `wget -O ~/admin/privcheck/ExploitSuggest.py http://www.securitysift.com/download/linuxprivchecker.py`
            `wget -O ~/admin/privcheck/LinEnum.zip https://github.com/rebootuser/LinEnum/archive/master.zip`
            `wget -O ~/admin/privcheck/ExploitSuggest_perl.zip https://github.com/PenturaLabs/Linux_Exploit_Suggester/archive/master.zip`
            `wget -O ~/admin/privcheck/unixprivesc.zip https://github.com/pentestmonkey/unix-privesc-check/archive/1_x.zip`
            `wget -O ~/admin/privcheck/firmwalker.zip https://github.com/craigz28/firmwalker/archive/master.zip`
             printf "%b \n"
            ;;
